...
Codex 전용 플러그인은 `.agents/plugins/marketplace.json`에 등록하고, 본체는 `plugins/<plugin-name>/`에 둔다. marketplace entry에는 `version` 필드를 두지 않고, `plugins/<plugin-name>/.codex-plugin/plugin.json`의 버전을 단일 기준으로 사용한다. 커밋 전 marketplace의 `source.path`, entry `name`, 플러그인 디렉토리 이름, manifest의 `name`이 모두 일치하는지 계약 테스트로 확인한다.
