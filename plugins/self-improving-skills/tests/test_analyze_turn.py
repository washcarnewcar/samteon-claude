...
def test_distillation_anchor_resets_counting(run_analyzer, sandbox):
    rows = _work_rows()
    rows.append(tool_use("Task", {"subagent_type": "self-improving-skills:skill-distiller",
                                  "prompt": "distill"}))
    assert run_analyzer(rows, "s")["decision"] == "approve"
...
def test_core_advisory_fires_once(run_analyzer):
    core = [tool_use("Edit", {"file_path": "/r/plugins/self-improving-skills/x.py"})]
...
def test_maintenance_segment_views_not_counted(run_analyzer, sandbox, store_data):
    ...
    run_analyzer([tool_use("Skill", {"skill": "self-improving-skills:curate-skills"}),
