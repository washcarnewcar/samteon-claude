...
- 스킬 트리와 함께 **usage 텔레메트리(`skill_usage.json`)와 팀 동기화 매니페스트(`team_sync.json`)도 스냅샷 시점으로 복원**합니다 — 트리만 수동으로 풀면 usage 레코드가 `state: archived` 로 남아 복원된 스킬이 라이프사이클에서 빠지고, origin_hash 가 새 트리와 어긋나 다음 /sync-team-skills 가 팀 스킬을 diverged 로 오인합니다.
