#!/usr/bin/env python3
"""Stop-hook analyzer for the self-improving-skills-cowork plugin.
...
Cowork adaptations (vs the original self-improving-skills analyzer):
  ...
  * The core-touch (L1) advisory routes to the samton-claude repo instead of
...
def _is_plugin_source_path(file_path):
    """True if a path is inside the self-improving-skills-cowork plugin's OWN
    ...
    improvement can be routed to the samton-claude repo instead of (only) into
    ...
    if "/.claude/skills/" in norm:
        return False
    return "/self-improving-skills-cowork/" in norm
...
def main():
    ...
    #       samton-claude repo (a Cowork container copy vanishes at session
    ...
    if nudge_fires or readonly_fires or core_touched:
        ...
        if nudge_fires or readonly_fires:
            msg = (
                ...
                '(subagent_type="self-improving-skills-cowork:skill-distiller" — 플러그인 '
        ...
        if core_touched:
            parts.append(
                "이번 구간이 self-improving-skills-cowork 코어 소스를 직접 수정했습니다. "
                "이건 ~/.claude/skills 증류와는 별개입니다 — Cowork 컨테이너 안의 플러그인 "
                "사본은 세션 종료 시 사라지고 다음 세션에는 claude.ai 에 등록된 버전이 다시 "
                "동기화되므로, 이 개선을 유지하려면 samton-claude 리포"
                "(plugins/self-improving-skills-cowork/)에 반영해야 합니다:\n"
