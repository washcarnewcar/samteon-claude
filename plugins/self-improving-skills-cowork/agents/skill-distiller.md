...
If the session you're distilling actually changed the **self-improving-skills-cowork
plugin's own source** (files under `plugins/self-improving-skills-cowork/`),
...
- THEN add to your report that the core change must be committed to the
  samton-claude repo to survive (the container's plugin copy is re-synced from
