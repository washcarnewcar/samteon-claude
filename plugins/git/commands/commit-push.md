---
allowed-tools: Bash(bash:*), Bash(git add:*), Bash(git status:*), Bash(git commit:*), Bash(git push:*), Bash(git branch:*), Bash(git log:*)
...
1. 그 목록의 파일만 stage 하여 단일 커밋을 만든다.
2. 커밋을 원격에 push 한다.
...
1. **그 목록의 파일만** `git add <path>`로 stage 한다. 작업트리의 다른 변경은 **절대 stage 하지
   ...
   쓰지 않는다.
2. 목록이 비어 있으면 커밋·push 하지 말고 그 사실만 한 줄로 보고하고 종료한다.
3. 커밋 메시지는 한국어 conventional commit `<type>(<scope>): <요약>` (버전 bump 시 `(vX.Y.Z)`).
4. 커밋 메시지에 co-author/trailer를 **넣지 않는다.**
