...
**Bash timeout 설정:**
- 일반 음성 메시지(~수 분): `timeout: 300000` (화자 분리 모델 로딩 포함)
- 긴 오디오(10분 이상): `run_in_background: true` 사용. 스크립트 자체에 시간 제한은 없다.
...
| Bash 도구 타임아웃 | Bash timeout을 늘리거나 `run_in_background: true`로 재실행. 스크립트 자체에는 시간 제한이 없다. |
...
| Bash timeout 기본값 사용 | 짧은 음성: Mode A 120000ms, Mode B 300000ms. 긴 오디오(10분+): `run_in_background: true` 사용 |
