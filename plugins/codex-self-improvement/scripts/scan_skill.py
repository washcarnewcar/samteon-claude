#!/usr/bin/env python3
"""Static scanner for Codex skill content (secrets / injection / leakage).

Skills are INSTRUCTIONS to an agent, i.e. a prompt-injection vector. Ported
from the sibling self-improving-skills plugin's team-boundary scanner (which
follows Hermes skills_guard/threat_patterns). In THIS plugin the scan is
advisory: skill_store runs it in report mode after create/patch/write and
folds the result into the returned dict — findings never block a local write
(Hermes ships guard_agent_created off by default for locally-created skills:
the agent can already run the same code through the shell, so blocking here
adds friction, not safety). If an external receive path (team sync) is added
later, THAT path should treat blocking findings as an install gate.

Modes:
  scan <dir>    gate mode. Prints a JSON report.
                exit 0 = clean or warnings only, exit 1 = blocking findings.
  report <dir>  advisory audit (always exit 0).

Severities:
  block  — secrets, private keys, destructive commands, prompt-injection
           markers, invisible unicode, symlinks, oversize, name mismatch
  warn   — machine-local absolute paths (routine in locally-authored skills)
"""
...
# Codex naming also allows dots/underscores (skill_store.VALID_NAME_RE).
NAME_RE = re.compile(r"^[a-z0-9][a-z0-9._-]*$")

# Bump when detection meaningfully strengthens.
SCANNER_VERSION = "csi-scan-v1"
