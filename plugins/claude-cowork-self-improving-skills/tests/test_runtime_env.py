...
that can reach both — a local install plus this variant synced from the
claude.ai account — would otherwise run both on one Stop, and the local session
would be told to press "스킬 저장" for a skill already written to a home that
outlives the session.

The conftest pins SIS_RUNTIME=cowork for every other test; the `unpinned`
...
def test_a_local_home_without_the_cli_marker_is_still_local(unpinned):
    ...
    made the claude-code variant stand down there and silently took the whole
    loop with it; this variant must not claim that session either.
    """
...
def test_home_is_re_read_on_every_call(unpinned, monkeypatch, tmp_path):
    ...
    local.mkdir()
    monkeypatch.setenv("HOME", str(local))
    assert runtime_env.is_cowork_runtime() is False
...
def test_the_stop_hook_stands_down_locally(run_analyzer, monkeypatch):
    """Work that nudges in Cowork must leave a bare approve on a local machine."""
    monkeypatch.setenv("SIS_RUNTIME", "local")
    r = run_analyzer(_work_rows(), "s")
    assert r == {"decision": "approve"}


def test_the_stop_hook_still_fires_in_cowork(run_analyzer):
    """The gate must not swallow the Cowork case it is meant to protect."""
    r = run_analyzer(_work_rows(), "s")
    assert r["decision"] == "block"
    assert "스킬 저장" in r["reason"]
