...
def test_nudge_includes_cowork_save_step(run_analyzer):
    r = run_analyzer(_work_rows(), "s")
    assert r["decision"] == "block"
    # the Cowork-critical delivery step must be part of every nudge
    assert "SendUserFile" in r["reason"] and "스킬 저장" in r["reason"]
    # and the distiller reference must carry the plugin namespace
    assert "claude-cowork-self-improving-skills:skill-distiller" in r["reason"]


def test_advisory_fallback_prepended_when_flag_missing(run_analyzer, sandbox):
    r = run_analyzer(_work_rows(), "s")
    assert r["decision"] == "block"
    assert "자기개선 루프 활성" in r["reason"]  # fallback advisory present
    assert (sandbox.home / ".claude" / "self-improve" / "advisory_shown").is_file()


def test_no_fallback_when_advisory_already_shown(run_analyzer, mark_advisory_shown):
    mark_advisory_shown()
    r = run_analyzer(_work_rows(), "s")
    assert r["decision"] == "block"
    assert "자기개선 루프 활성" not in r["reason"]  # no duplicate advisory
...
def test_distillation_anchor_resets_counting(run_analyzer, sandbox):
    rows = _work_rows()
    rows.append(tool_use("Task", {"subagent_type": "claude-cowork-self-improving-skills:skill-distiller",
                                  "prompt": "distill"}))
    assert run_analyzer(rows, "s")["decision"] == "approve"
...
def test_core_advisory_fires_once(run_analyzer):
    core = [tool_use("Edit", {"file_path": "/r/plugins/claude-cowork-self-improving-skills/x.py"})]
    r = run_analyzer(core, "s")
    assert r["decision"] == "block" and "코어 소스" in r["reason"]
    assert run_analyzer(core, "s")["decision"] == "approve"


def test_original_plugin_path_is_not_our_core(run_analyzer):
    # the ORIGINAL plugin's tree must not trip this variant's core advisory
    rows = [tool_use("Edit", {"file_path": "/r/plugins/claude-code-self-improving-skills/x.py"})]
    assert run_analyzer(rows, "s")["decision"] == "approve"


def test_core_advisory_cleared_by_repo_action(run_analyzer):
    rows = [tool_use("Edit", {"file_path": "/r/plugins/claude-cowork-self-improving-skills/x.py"}),
            tool_use("Bash", {"command": "git push origin main"})]
    assert run_analyzer(rows, "s")["decision"] == "approve"
...
def test_maintenance_segment_views_not_counted(run_analyzer, sandbox, store_data):
    ...
    run_analyzer([tool_use("Skill", {"skill": "claude-cowork-self-improving-skills:loop-status"}),
                  tool_use("Read", {"file_path": sp})], "maint")
    data = store_data()
    assert data.get("some-skill", {}).get("view_count", 0) == 0
    # /save-skill passes are maintenance too (they bulk-read SKILL.md files)
    run_analyzer([tool_use("Skill", {"skill": "claude-cowork-self-improving-skills:save-skill"}),
                  tool_use("Read", {"file_path": sp})], "maint2")
    assert store_data().get("some-skill", {}).get("view_count", 0) == 0
