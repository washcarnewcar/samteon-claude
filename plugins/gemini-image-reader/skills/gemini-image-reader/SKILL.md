...
description: |
  ...
  Gemini CLI is preferred for OCR, text extraction, and detailed visual analysis as it is more accurate
  than Claude's native image reading. If Gemini CLI is unavailable (not installed or not authenticated),
  fall back to Claude's built-in Read tool to view the image directly.
allowed-tools: Bash, Read
---

# Gemini Image Reader

이미지가 대화에 존재하면 Gemini CLI를 통해 읽는다. Gemini는 OCR, 텍스트 추출, UI 요소 식별에서 Claude 자체 비전보다 정확하다. Gemini CLI를 사용할 수 없는 경우 Claude의 Read 도구로 직접 분석한다.
...
최대한 디테일하게 분석해주세요.
표 형태의 데이터가 있으면 마크다운 표로 원본과 최대한 동일하게 재현해주세요.
텍스트는 한 글자도 빠짐없이 정확히 추출해주세요.
한국어로 답변해주세요.
```
...
Gemini CLI 가용성을 먼저 확인한다:
1. `which gemini` 또는 `/opt/homebrew/bin/gemini`로 설치 여부 확인
2. 설치되어 있으면 → wrapper 스크립트로 Gemini 호출 (아래 기존 동작)
3. 미설치이거나 실행 실패(인증 오류 등) 시 → **Gemini 사용 불가 시: Claude Fallback** 진행
...
Gemini CLI가 미설치이거나 실행 실패(인증 오류, 타임아웃 등)인 경우:

1. Read 도구로 이미지 파일을 직접 읽는다 (Claude의 멀티모달 비전 사용)
2. Step 1에서 구성한 프롬프트의 분석 컨텍스트를 적용하여 직접 분석한다
3. 사용자에게 Gemini 대신 Claude 비전으로 분석했음을 간단히 알린다

fallback 시에도 Step 1의 컨텍스트 구성 원칙은 동일하게 적용한다.
...
| 상황 | 대응 |
|------|------|
| Gemini CLI 미설치 | Claude Read 도구로 직접 분석 + `npm install -g @google/gemini-cli` 안내 |
| 인증 실패 (AuthError) | Claude Read 도구로 직접 분석 + 터미널에서 `gemini` 대화형으로 재인증 안내 |
